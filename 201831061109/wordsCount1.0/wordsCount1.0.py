...
class word_operate():
    ...
    def wordgroup(self, m):
        wordlist = self.contents.split()  # 单词列表
        i = 0
        n = m
        countgroup = {}  # 词组储存
        while i < len(wordlist) and (i + m - 1) < len(wordlist):
            # print (f'{wordlist[i:i + m]} ')
            key = ' '.join(wordlist[i:i + m])  # 将词组作为字典key
            i += 1
            countgroup[key] = countgroup.get(key, 0) + 1  # 词组频率统计
        for key in countgroup:
            if isSave == False:
                print(f'{key}:{countgroup[key]}\n')
            else:
                print(f'{key}:{countgroup[key]}\n')
                doc = open(f'{outfile}', 'a')
                print(f'{key}:{countgroup[key]}\n', file = doc)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "i:o:m:n:")
    except getopt.GetoptError:
        print('test.py -i <filepath> -m <number_of_phrase> -n <number_of_words>'
              '-o <outfile_path> -m <number_of_phrase>')
        sys.exit(1)
    ...
    for opt, arg in opts:
        if opt == '-i':
            ...
            if arg.isdigit():
                ...
                wordgroup = word_operate(txt_file, contents, filename)
                wordgroup.wordgroup(int(arg))
            else:
                print("-m para is integer!")

    if filename == '':
        print("Please at least enter the '-i' parameter!")
        print('Example==>"python test.py -i <filepath> -m <number_of_phrase>' ''
              '-n <number_of_words> -o <outfile>"')
        sys.exit(0)
